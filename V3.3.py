...
class BookFinder:
    ...
    #Add for fiction book 1#
    def fb1_add(self):
        global afb1
        afb1 += 1
        f1avalbl = tk.Label(self.fic1dtl,bg="#dae8fc")
        f1avalbl.place(x=600,y=300)
        f1avalbl.config(font=("Arial",32),text=afb1)

    def fb1_min(self):
        global afb1
        afb1 -= 1
        if afb1 < 0:
            mb.showwarning("Too Less","Amount of book can not be negative number.")
            afb1 +=1
        else:
            f1avalbl = tk.Label(self.fic1dtl,bg="#dae8fc")
            f1avalbl.place(x=600,y=300)
    ...
    def login_widgets(self):
        ...
        #Close Button#
        self.clsbtn = tk.Button(self.lgnframe,fg="white",bg="#1ba1e2",font=("Arial",22),text="Close",command=window.destroy)
    ...
    def detail_widgets(self):
        ...
        self.clsprgrmbtn = tk.Button(self.dtlframe,fg="white",bg="#1ba1e2",font=("Arial",22),text="Close",command=window.destroy)
    ...
    def booktype_widgets(self):
        ...
        bkbtn = tk.Button(self.bktpframe,fg="white",bg="#1ba1e2",font=("Arial",22),text="back",command=self.detail_frame)
        bkbtn.place(x=50,y=450)
    ...
    def f1ava_widget(self):
        ...
        f1avalbl = tk.Label(self.fic1dtl,bg="#dae8fc",font=("Arial",32),text="1")
        f1avalbl.place(x=600,y=300)
        #Add Button#
        addbtn = tk.Button(self.fic1dtl,fg="white",bg="#1ba1e2",width=4,font=("Arial",32),text="+",command=self.fb1_add)
        addbtn.place(x=800,y=300)
        #Minus Button#
        minbtn = tk.Button(self.fic1dtl,fg="white",bg="#1ba1e2",width=4,font=("Arial",32),text="-",command=self.fb1_min)
        minbtn.place(x=400,y=300)
        #Back button#
        bkbtn = tk.Button(self.fic1dtl,fg="white",bg="#1ba1e2",text="Back",command=self.fbook1_frame)
        bkbtn.place(x=370,y=490)
        #Change availability button#
        savebtn = tk.Button(self.fic1dtl,fg="white",bg="#1ba1e2",text="Save",command=self.fb1_save)
        savebtn.place(x=820,y=490)

    def f2dtl_widget(self):
        #Detail Frame Of Fiction Book 1#
        #Large Image#
        image = Image.open("fiction_TheWarHero.png")
        resize_image = image.resize((270,400))#108*2.5=270,162*2.5=405,take whole number#
        img = ImageTk.PhotoImage(resize_image)
        largeimg = tk.Label(self.fframeb2,image=img)
        largeimg.image = img
        largeimg.place(x=50,y=70)#(540-400)/2=70#
        #Title Label#
        authlbl = tk.Label(self.fframeb2,bg="#dae8fc",font=("Arial",22),text="The War Hero",justify="left")
        authlbl.place(x=370,y=70)
        #Author Label#
        authlbl = tk.Label(self.fframeb2,bg="#dae8fc",font=("Arial",16),text="Author: Joseph Elliott",justify="left")
        authlbl.place(x=370,y=110)
        #Genre Label#
        genlbl = tk.Label(self.fframeb2,bg="#dae8fc",font=("Arial",16),text="Genre: Fiction",justify="left")
        genlbl.place(x=370,y=140)
        #Publisher Label#
        publbl = tk.Label(self.fframeb2,bg="#dae8fc",font=("Arial",16),text="Publisher: Walker Books Us",justify="left")
        publbl.place(x=370,y=170)
        #Description#
        deslbl = tk.Label(self.fframeb2,bg="#dae8fc",font=("Arial",14),text="After their escape from Norveg, Agatha and Jaime return\nwith their clan to the Isle of Skye to find that their enclave\nis now in the hands of the treacherous people of Raasay.\nThey find tenuous shelter with another clan, but disaster\nsoon strikes when the terrifying shadow creatures known\nas sgàilean escape their magical prison and wreak havoc\nacross the island. Now Agatha and Jaime must call on old\nand new allies to fight this threat. In the meantime, a ship\nfrom Norveg sails for the court of King Edmund of Ingland,\nwhere a dangerous alliance is forming, and Sigrid, a girl with\nan extraordinary memory, works to free herself from the\nclutches of a cruel king.",justify="left")
        deslbl.place(x=370,y=200)
        #Back button#
        bkbtn = tk.Button(self.fframeb2,fg="white",bg="#1ba1e2",text="Back",command=self.fiction_frame)
        bkbtn.place(x=370,y=490)
        #Change availability button#
        availbtn = tk.Button(self.fframeb2,fg="white",bg="#1ba1e2",text="Check Availability",command=self.f2dtl_frame)
        availbtn.place(x=820,y=490)
...
if __name__ == "__main__":
    ...
    window.title("Book Inventory V3.3")
